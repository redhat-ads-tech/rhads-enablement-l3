...
NOTE: The xref:setup-rhdh/foundations.adoc[Setup Red Hat Developer Hub] module provides an in-depth overview of {product_rhdh_name}'s dynamic plugins feature. Visit that module if you'd like a deep-dive on deployment and configuration for {product_rhdh_name}.
...
----
. Before proceeding, change `disabled: true` to `disabled: false` for the listed plugins, and click *Save*. 
...
. Return to the https://console-openshift-console.{openshift_cluster_ingress_domain}/topology/ns/{ns_tssc_rhdh}?view=graph[Topology View^] for the *{ns_tssc_rhdh}* namespace and wait for the SonataFlowPlatform pods to become ready (dark blue circle). This may take a few minutes for both the Orchestrator plugins and SonataFlowPlatform pods to become ready.
