...
IMPORTANT: There are configuration properties that you need to take care of (that go beyond the default client definition), should you build or configure a TAS OIDC Client: The *Redirect URLs* and three *Client Scopes*:
...
[NOTE]
====
The `urn:ietf:wg:oauth:2.0:oob` URI is used when an OAuth client cannot receive the authorization response through a standard HTTP redirect. Instead of redirecting the user back to your application with the authorization code, the authorization server displays the code directly to the user, who must then manually copy it and paste it into your application.
...
* `email-verified` which needs to be mapped to the user profile `emailVerified` property
* `email` which needs to be mapped to the user profile `email` property

* `aud` (Audience) which needs to be hard-coded (yes!) to `trusted-artifact-signer`. *This needs to be part of the access token*
...
Either way, all the following instructions will use this namespace (and URLs generated from this namespace).
...
Open the operator and create a "Securesign" here in the `student-tpa` namespace. It should automatically switch to the YAML view, where we have to set a few parameters - in the order of importance:
...
Create your instance, then open it and wait a moment until all Conditions (especially the top "Ready") are `true`
...
You should now have a new terminal https://admin-terminal-ttyd.{openshift_cluster_ingress_domain}[here^,window="terminal"] that we will be using for the verification steps. 
...
[source,console]
----
podman-terminal:/workspace$ echo "FROM scratch" > ./tmp.Dockerfile
podman build . -f ./tmp.Dockerfile -t ttl.sh/rhtas/test-image:1h
podman push ttl.sh/rhtas/test-image:1h
STEP 1/1: FROM scratch
COMMIT ttl.sh/rhtas/test-image:1h
--> fc8cf8b1a7d7
Successfully tagged ttl.sh/rhtas/test-image:1h
fc8cf8b1a7d734c6e93ab3818cfad01a351380f3042586daf6428f64f7cdd2e2
Getting image source signatures
Copying config fc8cf8b1a7 done   | 
...
[source,console]
----
podman-terminal:/workspace$ cosign sign -y ttl.sh/rhtas/test-image:1h
...
Go to the following link in a browser:
         ...
         https://sso.apps.cluster-mdt2d.dynamic.redhatworkshops.io/realms/trusted-artifact-signer/protocol/openid-connect/auth?access_type=online&client_id=trusted-artifact-signer&code_challenge=B6VZJhDr089zfOYICLLaoYy4xNchsCdKlIs1pmFCcgU&code_challenge_method=S256&nonce=34YPEYzx0zY3llKy4LQDbXZnu3y&redirect_uri=urn%3Aietf%3Awg%3Aoauth%3A2.0%3Aoob&response_type=code&scope=openid+email&state=34YPEXamVdsNOwbVkcZFnkt2J0C
...
[source,console]
----
Enter verification code: e9227e1a-0f27-47b7-a05f-c2b27f462e65.1808684f-e7ac-4c9a-822e-bfdcbdd53927.033e6140-f6cb-4613-b729-40cbfd0e0821

Successfully verified SCT...
WARNING: Image reference ttl.sh/rhtas/test-image:1h uses a tag, not a digest, to identify the image to sign.
...
Pushing signature to: ttl.sh/rhtas/test-image
---- 
...
[source,console]
----
podman-terminal:/workspace$ cosign tree ttl.sh/rhtas/test-image:1h
📦 Supply Chain Security Related artifacts for an image: ttl.sh/rhtas/test-image:1h
└── 🔐 Signatures for an image tag: ttl.sh/rhtas/test-image:sha256-2db91f12665bc42032882f65565b1a0913f5e2d4a101f4c62556cc8c5cbb5174.sig
   └── 🍒 sha256:35e65f39048d76a6edce96f036304446e9ba0a250807a67d9aa7282d6efea5c5
---- 
...
[source,bash,role=execute,subs=attributes+]
----
cosign verify --certificate-identity=user1@demo.redhat.com ttl.sh/rhtas/test-image:1h | jq
---- 
(if you signed with the admin user, you will get an error - use `admin@demo.redhat.com` instead)
...
[source,bash,role=execute,subs=attributes+]
----
cosign verify --certificate-identity user1@demo.redhat.com --certificate-oidc-issuer https://sso.{openshift_cluster_ingress_domain}/realms/trusted-artifact-signer ttl.sh/rhtas/test-image:1h | jq
---- 
...
[source,bash,role=execute,subs=attributes+]
----
cosign verify --certificate-identity-regexp '.*@.*redhat\.com$' --certificate-oidc-issuer-regexp '\.redhatworkshops\.io/' ttl.sh/rhtas/test-image:1h | jq
---- 
...
IMPORTANT: Please make sure your TAS endpoints are still configured to point to your `student-tas` namespace deployment by typing `'help'`. Otherwise you'll verify that the pre-configured lab environment is working (which we sure hope it is... 😉 )
...
└── 🔐 Signatures for an image tag: ttl.sh/rhtas/test-image:sha256-2b729297e54dde40b262b1569ac5327584b084ac2e25a1a0d5a1d7e173edc2fe.sig
   └── 🍒 sha256:e25719b2ac6cdee3c2fc21d655f3db90b3be4223b8bc2b3ef406a990e680bfd2
----
...
[source,bash,role=execute,subs=attributes+]
----
ec validate image --image ttl.sh/rhtas/test-image:1h --certificate-identity=user1@demo.redhat.com --certificate-oidc-issuer=$SIGSTORE_OIDC_ISSUER --show-successes
----

[source,console]
----
podman-terminal:/workspace$ ec validate image --image ttl.sh/rhtas/test-image:1h --certificate-identity=user1@demo.redhat.com --certificate-oidc-issuer=$SIGSTORE_OIDC_ISSUER --show-successes
Success: true
Result: SUCCESS
...
[source,bash,role=execute,subs=attributes+]
----
ec validate image --image ttl.sh/rhtas/test-image:1h --certificate-identity=user1@demo.redhat.com --certificate-oidc-issuer=$SIGSTORE_OIDC_ISSUER  --output json --show-successes | jq
----
