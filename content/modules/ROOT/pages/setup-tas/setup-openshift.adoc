...
cosign verify --certificate-identity=user1@demo.redhat.com \# <1>
       $IMAGE_NAME | jq
---- 
<1> If you signed with the admin user, you will get an error - use `admin@demo.redhat.com` instead by pressing the kbd:[⬆️] key and editing the username in the command. You will need to do this for all subsequent commands that use the `user1@demo.redhat.com` identity.
