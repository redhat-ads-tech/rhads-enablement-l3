...
TPA requires a postgresql database for the graph database. We have prepared a `yaml` file with the required k8s artifacts, that you can simply apply. 
...
After saving (or later, opening a Scope from the Client Scopes list), you can assign a role under the _Scope_ tab:

image:m3-tpa-openshift/keycloak-assign-roles.png[]

Assign all roles to the `read:document` scope
...
2) The https://console-openshift-console.{openshift_cluster_ingress_domain}/k8s/ns/student-tpa-operator/deployments/server/pods[server pod^,window="console"] (click on the name of the importer pod, then click *Logs*):
...
. In the `storage.region` field, replace `<your-cluster-ingressdomain-here>` with `{openshift_cluster_ingress_domain}` to look like this:
+
...
. In the `oidc.issuerUrl` field, also replace the cluster ingress domain with `{openshift_cluster_ingress_domain}` to look like this:
+
