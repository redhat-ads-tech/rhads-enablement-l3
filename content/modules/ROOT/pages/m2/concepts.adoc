...
The `output` values are documented on a per plugin basis. You can find the values for the specific version of your installed plugins by accessing the https://backstage-{m2_rhdh_instance}-{m2_rhdh_project}.{cluster_apps_domain}/create/actions[/create/actions] endpoint on your {product_rhdh_name} instance.
