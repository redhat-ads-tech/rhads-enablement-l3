...
. Docs reside with the Component source code in the Component's source code repository, conventionally in https://manpages.debian.org/trixie/libmarkdown2-dev/markdown.7.en.html[Markdown^] format in a directory named `docs`.
...
Note, however, that {product_rhdh_name} documentation clearly states that the https://docs.redhat.com/en/documentation/red_hat_developer_hub/1.7/html/techdocs_for_red_hat_developer_hub/configuring-techdocs[included lightweight TechDocs configuration is not intended for production^] use: in addition to the ephemeral nature of the storage, there is a limit on the maximum number and size of documents that can be rendered and stored, which will quickly be exceeded in a production environment. We'll revisit this topic in a later lab.
...
In the appearance customization lab, you worked in the OpenShift Project `brand-rhdh` and created a new {product_rhdh_name} instance. In this lab, you'll begin with a provided Developer Hub already running in a another Project, named `tssc-dh`.  This instance has been configured with a default but fully functional TechDocs system 

. On the OpenShift Web Console, Select the Administrator perspective, navigate to _Workloads_ > _ConfigMaps_, and ensure the `tssc-dh` Project is selected in the project selector drop-down.

. Find and click on the `tssc-developer-hub-app-config` ConfigMap to view the `app-config.yaml` file that contains the TechDocs process configuration for this pre-installed instance of {product_rhdh_name}.
+
