...
Continuing with the pre-installed Developer Hub instance running in the OpenShift Project `tssc-dh`, the steps that follow illustrate the intended user experience for developers reading and writing TechDocs and demonstrate the operation of the default _local_-context TechDocs included with RHDH and detailed in the preceding xref:techdocs-foundations.adoc[TechDocs Foundations].
...
[NOTE]
====
Even when components are unregistered from the {product_rhdh_name} Catalog, any external resources (e.g. GitLab repositories, OpenShift projects, etc.) associated with the component may still exist. If another component is later created with the same name (e.g. `uq`) by the same user (e.g. `user1`), resources may fail to be created due to conflicts.
====
