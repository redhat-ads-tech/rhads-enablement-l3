...
Red Hat Advanced Cluster Security for Kubernetes works with Kubernetes admission controllers and OpenShift Container Platform admission plugins that allow to enforce security policies before Kubernetes or OpenShift Container Platform creates workloads, for example, deployments, daemon sets or jobs.
...
* ACS can verify that an admissible image has been signed via `cosign` / Tusted Artifact Signer and verify that against the TAS Rekor database
...
However, the image signature is the only parameter checked, so it only provides a subset of the verification options that the Admission Controllers offer.
