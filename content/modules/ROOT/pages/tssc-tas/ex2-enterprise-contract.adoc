...
Rego is a declarative policy language developed by the Open Policy Agent (OPA) project. It's designed specifically for expressing policies as code and making authorization and validation decisions across various enviroinment and in multiple scenarios. +
...
Unlike imperative programming languages that describe _how_ to compute something, Rego is declarative—you describe _what_ the desired outcome should be, and the engine figures out how to evaluate it.
