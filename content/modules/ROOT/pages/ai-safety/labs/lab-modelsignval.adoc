...
* *Namespace (model-signing)*: During this lab, you will be using the *model-signing* namespace.
* *PersistentVolumeClaim (model-storage-pvc)*: Requests 5Gi of storage for the model files.
* *Job (download-model)*: Downloads a sample TensorFlow SavedModel.
...
spec:
  ...
  resources:
    requests:
      storage: 5Gi
  volumeName: pvc-8af02176-3d32-4bf3-8793-249453eeee57
        ...
        - name: download-model
          ...
          command:
            ...
            - |
              ...
              wget -O /models/saved_model.pb https://github.com/redhat-ads-tech/rhads-enablement-l3/raw/refs/heads/main/content/modules/ROOT/solutions/ai-safety/models/saved_model.pb
          volumeMounts:
            - name: model-storage
              mountPath: /models
      restartPolicy: Never
...
----
+
Review the output. You should see output similar to:
+
image:ai-safety/msv-terminal.png[Pod Terminal, link=self, window="image"]
+
Copy the URL provided by sigstore *model-signing* and open it in your web browser. Choose *Google* from the list of identity providers: Google, GitHub, Microsoft.
+
...
This file will be used by the Model Validation Operator to verify the model hasn't been tampered with and the model that will run on the cluster is actually signed.
====
...
The Model Validation Operator uses a Mutation Webhook to inject an init container on the pod that will run the model. This allows the Model Validation Controller to know what pod/model needs to be verified.
...
The ModelValidation CRD tells the operator which models to validate and which OIDC identity is authorized to sign them. Let's review the key fields:
...
. Select *YAML* as the configure via. 
...
. Copy the following InferenceService definition:
+
[source,yaml,role=execute,subs=attributes+]
----
apiVersion: v1
...
. Navigate to the pods view in your cluster:
+
https://console-openshift-console.{openshift_cluster_ingress_domain}/k8s/ns/model-signing/pods[Pods^]

. Click on the *model-server* pod

. Check the pod's *Logs -> Init Containers*. You should see a container named `model-validation`.

. Click on the *model-validator* init container and view its logs. You should see:

+
image::ai-safety/msv-pod-running.png[link=self, window="image"]

. Verify the main container started successfully by checking the pod status shows *Running*
...
To see validation in action, you can use another folder where it is empty, you will create a new pod with a new *mountPath:/path*:
