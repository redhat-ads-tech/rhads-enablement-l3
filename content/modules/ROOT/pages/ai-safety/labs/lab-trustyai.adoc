...
In the *OpenShift Web Console*, click on *Operators -> Installed Operators* and select the *RH OpenShift AI* operator or use the following URL  {openshift_console_url}/k8s/ns/redhat-ods-operator/operators.coreos.com~v1alpha1~ClusterServiceVersion/rhods-operator.2.25.1[Web Console^]
...
+
. Go to line *157* and search for the *trustyAI* component.
...
. In the OpenShift Web console, create a new project in the cluster, click on *Home -> Projects -> Create Project* with the name: *model-namespace*
+
...
✅ *Success!* You have successfully configured TrustyAI. The service is now ready to monitor model predictions for bias. Next, you'll deploy the loan prediction models and configure them to send inference data to TrustyAI for continuous fairness monitoring.
...
[IMPORTANT]
====
You will see the deployments being updated. Expect the new 4-container deployment to briefly coexist with the old 3-container version.
====
+
...
. In OpenShift, use the *OpenShift Web Terminal*
...
[TIP]
====
While the script outputs confirmation messages indicating data reception by TrustyAI, you can also verify the stream in the OpenShift web console.
====
...
. In the *Expression field*, enter the Prometheus metric: `trustyai_model_observations_total`

. Set the time window to **5 minutes** (top left), click *Run queries* and the refresh interval to **15 seconds** (top right)
+
image:ai-safety/query-new-added.png[Metrics, link=self, window="image"]

. After running the query, confirm that both models are listed, each reporting close to **2250 observed inferences**
+
...
TRUSTY_ROUTE=https://$(oc get route/trustyai-service -n model-namespace --template={{.spec.host}})
...
// Sample output (abbreviated)
      ...
      "customer_data_input-9": {
        "type": "DOUBLE",
        "name": "customer_data_input-9",
        "columnIndex": 9
      },
      "customer_data_input-8": {
        "type": "DOUBLE",
        "name": "customer_data_input-8",
        "columnIndex": 8
      },
...
[TIP]
====
From these results, we see that both model Alpha and Beta are quite fair over the Is Male-Identifying? field, with the two groups' rates of positive outcomes only differing by -0.3% and 2.8% respectively.
====
...
Let's check the results. Let's first look at our two models' fairness.

=== Models' Fairness

. Query for identity metrics:
+
. Clear the expression field and type: `trustyai_spd`
+
. You should see metrics like:

image:ai-safety/metrics-spd-updated.png[Metrics, link=self, window="image"]

Immediately, we notice that the two models have drastically different fairnesses over the real world data. Model Alpha (blue) remained within the "acceptably fair" range between -0.1 and 0.1, ending at around 0.09. However, Model Beta (orange) plummeted out of the fair range, ending at -0.274, meaning that non-male-identifying applicants were 27 percent less likely to get a favorable outcome from the model than male-identifying applicants; clearly an unacceptable bias.
...
[NOTE]
====
Ensure you have unchecked: *Will Default?* while *Is Male-Identifying?* remains checked.
====

. You should see metrics like:

image:ai-safety/trustyai_identity-updated.png[Metrics, link=self, window="image"]
...
[NOTE]
====
Ensure you have checked: *Will Default?* while *Is Male-Identifying?* is unchecked.
====

image:ai-safety/trustyai_identity-last.png[Metrics, link=self, window="image"]


We can see that despite seeing only non-male applicants, Model Alpha (green) still provided varying outcomes to the various applicants, predicting "will-default" around 25% of the time. Model Beta (blue) predicted "will-default" 100% of the time: every single applicant was predicted to default on their loan. Again, this is a clear indicator that our model is performing poorly on the real-world data and/or has encoded a systematic bias from its training; it is predicting that every single non-male applicant will default.
