...
2- The RHTAS Certificate Authority will issue a certificate for the model based on the identity token.
3- RHTAS Transparency Log receives the Signed Model Certificate.
4- The RHTAS Transparency Log adds detailed information received from the certificate for traceability as part of the Log inclusion proof. This information is returned and now it’s part of the Signed Model Certificate Proof
5- The Signed Model Certificate Proof is now part of the Model and will be deployed with the model in the Model Hub.
6- Once there's a request, the RHTAS Controller will verify that the model is signed before serving the model.
7- The model signed is served.

image::ai-safety/model-signing-validation.png[]
...
image::ai-safety/model-signing.png[]
