...
image:rhads-ai/rhads/rhdh-catalogicon.png[width=30%,link=self, window="image"]

* Next, click on *Self-service*:

image:rhads-ai/rhads/rhdh-selfservice.png[width=30%,link=self, window="image"]

Then select the software template: *Python - Trusted Application Pipeline*

* Click on *Choose*:

image:rhads-ai/rhads/rhdh-catalog-python.png[width=60%,link=self, window="image"]
...
image:rhads-ai/rhads/rhdh-python-user.png[width=60%,link=self, window="image"]

Click on *Next*:

image:rhads-ai/rhads/rhdh-python.png[width=100%,link=self, window="image"]
...
image:rhads-ai/rhads/rhdh-gitselection.png[width=30%,link=self, window="image"]
...
image:rhads-ai/rhads/rhdh-python2.png[width=100%,link=self, window="image"]
...
image:rhads-ai/rhads/rhdh-python3.png[width=100%,link=self, window="image"]
...
image:rhads-ai/rhads/rhdh-python4.png[width=100%,link=self, window="image"]
...
image:rhads-ai/rhads/rhdh-python5.png[width=100%,link=self, window="image"]
...
Learn more about these templates at https://docs.redhat.com/en/documentation/red_hat_advanced_developer_suite$$_-$$_software_supply_chain/latest/html/customizing_red_hat_advanced_developer_suite$$_-$$_software_supply_chain/customizing-sample-pipelines_default[Customizing Sample Pipelines^].

== Adding more configurations

You have received new requests from the AI Engineering team:

. Update the *devfile.yaml* file with application configurations. 
...
* Log in to GitLab using your credentials by clicking on *Sign in*:
+
image:rhads-ai/rhads/gitlab-sign-in.png[width=100%,link=self, window="image"]
+
...
+
image:rhads-ai/rhads/gitlab-open.png[width=100%,link=self, window="image"]
...
* Commit your changes:
+
image:rhads-ai/rhads/gitlab-commit.png[width=60%,link=self, window="image"]
...
+
image:rhads-ai/rhads/gitlab-catalog.png[width=60%,link=self, window="image"]
...
[source,yaml,role=execute,subs=attributes+]
----
    - url: https://devspaces.{openshift_cluster_ingress_domain}#https://gitlab-gitlab.{openshift_cluster_ingress_domain}/rhdh/ai-agent
...
+
image:rhads-ai/rhads/ai-agent-catalog-info.png[width=100%,link=self, window="image"]
...
+
image:rhads-ai/rhads/rhdh-refresh-component.png[width=60%,link=self, window="image"]
...
image:rhads-ai/rhads/gitlab-webhook.png[width=80%,link=self, window="image"]

* Click *Add new webhook* 

image:rhads-ai/rhads/gitlab-webhook-new.png[width=100%,link=self, window="image"]
...
** Comments
+
image:rhads-ai/rhads/gitlab-webhook-config.png[width=100%,link=self, window="image"]

* Scroll to the bottom and click *Add webhook*.
+
image:rhads-ai/rhads/gitlab-add-webhook.png[width=100%,link=self, window="image"]
...
+
image:rhads-ai/rhads/gitlab-webhook-test.png[width=100%,link=self, window="image"]
...
The TSSC pipeline uses OpenShift Pipelines to build a container image, generate an SBOM, and push both to Red Hat Quay. Subsequent tasks use the Red Hat Advanced Cluster Security (ACS) *roxctl* CLI:

* *acs-image-check* task verifies the image signature against security policies, and
* *acs-image-scan* task identifies vulnerabilities.

Red Hat Quay acts as the secure image registry, storing the signed images, SBOMs, and metadata. The deployment object will be updated with the new image tag, this make take ~5 minutes to get synchronize by Argo CD.
...
+
image:rhads-ai/rhads/ai-agent-deployment.png[width=100%,link=self, window="image"]
