...
== Scenario 
You are part of the AI Engineering team and you will leverage the tools setup by the AI Platform Engineering team including best practices—aligned with your organization's guidelines—for the AI team to build an AI agent in OpenShift.

Let's get started.

== Understanding software templates

Software templates provide the manifests to build, deploy, and promote any application that runs in a container or as a Kubernetes component. They ensure development teams start with established best practices and organizational guidelines, putting guardrails and compliance in place before the application ever runs on the cluster.
...
** On the same screen or go to {rhdh_url}/catalog/default/component/ai-agent/[Red Hat Developer Hub UI - AI Agent^]. 
** Click on *Source Repository* 
...
* *Note:*  it might take some minutes to show the new link. To speed it up access the template can refresh the entity using the refresh icon in the entity's About box, then refresh the full RHDH UI
