...
image::setup-rhdh/security-deny-by-default.png[link=self, window="image"]
...
+
image::setup-rhdh/security-admin-catalog-access.png[link=self, window="image"]
. Next, select *Administration > RBAC* item in the side menu - the RBAC UI should be displayed.
+
image::setup-rhdh/security-rbac-view-admin.png[link=self, window="image"]
...
. Configure the policy with the following settings:
  ...
  * *Owner*: Empty
+
image::setup-rhdh/security-rbac-policy-name-desc.png[link=self, window="image"]
+
[NOTE]
====
...
image::setup-rhdh/security-tssc-group.png[link=self, window="image"]
====
. Click *Next*.
. Select the `tssc` group to apply this policy to.
+
image::setup-rhdh/security-rbac-policy-group.png[link=self, window="image"]
. Click *Next*.

=== Configure Policy Permissions
. In the *Add permission policies*, select the *Catalog* plugin.
+ 
image::setup-rhdh/security-rbac-policy-plugins.png[link=self, window="image"]
. Expand
...
+
image::setup-rhdh/security-rbac-policy-plugin-settings.png[link=self, window="image"]
. Click *Next* to review your new policy.
+
image::setup-rhdh/security-rbac-policy-review.png[link=self, window="image"]
...
+ 
image::setup-rhdh/security-restricted-user1.png[link=self, window="image"]
...
----
+
image::setup-rhdh/security-casbin-cr.png[link=self, window="image"]
. Click *Save*.
...
+
image::setup-rhdh/security-casbin-roles-list.png[link=self, window="image"]
. Click on the `location_read` policy and confirm that the *Modified by* column states that it's managed by the policy file.
+
image::setup-rhdh/security-casbin-role.png[link=self, window="image"]
