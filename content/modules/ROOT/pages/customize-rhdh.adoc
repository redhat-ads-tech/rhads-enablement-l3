...
. Paste the copied ConfigMap YAML into the empty text field, then click *Create* to commit the `brand-rhdh` ConfigMap.
...
[NOTE]
====
FYI: Create ConfigMap on the command line

_Do not perform these steps if you've already created the `rhdh-config` ConfigMap in the OpenShift Web Console._

You can create Resources like the `brand-rhdh` ConfigMap on the command line. This requires:
...
In the shell session below, _cat_(1) creates and opens a new file, `app-config.yaml`, then waits for input. You can paste in your ConfigMap YAML, then type *Control-D* (`^D`) to signal the end of the file. _Cat_ writes the file, closes it, and exits. Then _oc_(1) uses app-config.yaml as the definition of a new ConfigMap. 
...
[NOTE]
====
FYI: Create CustomResource on the command line

_Do not perform these steps if you've already created the `rhdh` CustomResource in the OpenShift Web Console._
...
backstage.rhdh.redhat.com/rhdh created
----
====
