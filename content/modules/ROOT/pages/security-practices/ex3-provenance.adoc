...
Attesting this follows the exact same approach, but we'll use the `cosign` built-in predicate type. 
...
Using `cosign download attestation` will give us both (all), but we can filter by type:
