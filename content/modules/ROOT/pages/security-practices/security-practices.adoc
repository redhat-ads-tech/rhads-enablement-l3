...
* What vulnerabilities exist (if _used_ in e.g. Trusted Profile Analyzer - the SBOM itself doesn't contain vulnerability information)
...
IMPORTANT: Tools like Enterprise Contract / Conforma (part of Trusted Artifact Signer) can automate the validation of attestation and artifact signatures and subsequent policy enforcement (applying policies to attestation data). This drastically minimises the complexity of these verification steps.
