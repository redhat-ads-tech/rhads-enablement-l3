...
We will upload and manage SBOMs in xref:tssc-tpa/tssc-tpa.adoc[Module 8: Trusted Software Supply Chain with TPA] but if you'd like to take a quick look, we'll have to format it (`jq`) and make it accessible (`less`). 
...
image::security-practices/acs-integrations.png[link=self, window="image"]
...
image::security-practices/quay-signed-image-attestations.png[link=self, window="image"]
...
In the previous chapter (and to more extent in xref:tssc-tas/tssc-tas.adoc[Module 9: Attestations, Tekton Chains & Enterprise Contract with TAS]) we used Enterprise Contract / Conforma for the basic verification.
